...
export default function Comment({ comment, postId }) {
  const hasBeenLiked = comment.usersLiked.find(
    user => user.id === Auth.getProfile().data._id
  )
    ? true
    : false;
  ...
  const handleLike = async event => {
    ...
    try {
      ...
      setCommentLikes(
        updatedComment.data.updateCommentLikes.comments.find(
  ...
  return (
    <Box display="flex" bg="gray.100">
      <Container display="grid" gap="20px" py={12}>
        <Box
        ...
        >
          {comment.image && (
            <Box
              ...
              pos={'relative'}
            >
              <Image src={comment.image} layout={'fill'} />
            </Box>
          )}
          <Stack mb={3}>
            <Text
